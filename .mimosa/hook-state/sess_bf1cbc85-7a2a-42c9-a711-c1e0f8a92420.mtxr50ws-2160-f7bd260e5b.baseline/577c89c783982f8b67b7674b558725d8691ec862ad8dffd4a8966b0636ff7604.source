...
# ── ①a DEFAULT_SETTINGS 清简 ──
old = "  const DEFAULT_SETTINGS = { apiKey: '', model: 'glm-4-flash', provider: 'zhipu', baseUrl: '' };"
new = "  const DEFAULT_SETTINGS = {};   // 其余（apis/activeApi）由迁移生成"
assert old in s, '①a 未匹配'
s = s.replace(old, new)
print('①a 应用')

# ── ①b PROVIDERS → API_PRESETS（厂商名/地址/预设模型档案） ──
old = """  const PROVIDERS = {
...
new = """  // 各厂商 OpenAI 兼容接口：预填地址与常用模型档案（一个厂商 = 一套独立 API + 模型列表）
  const API_PRESETS = {
...
assert old in s, '①b PROVIDERS 未匹配'
s = s.replace(old, new)
print('①b 应用')

# ── ② seedModelsIfNeeded → migrateModelSettings ──
